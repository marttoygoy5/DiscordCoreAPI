	...
	namespace ClientClasses {
		...
		public:
			...
			task<void> deleteUserReactionAsync(DataManipFunctions::DeleteReactionData deleteReactionData) {
				DataManipFunctions::DeleteReactionData deleteReactionDataReal = deleteReactionData;
				deleteReactionDataReal.messageId = this->messageId;
				deleteReactionDataReal.channelId = this->channelId;
				
				string emoji;
				if (deleteReactionDataReal.emojiId != string()) {
					emoji += ":" + deleteReactionDataReal.emojiName + ":" + deleteReactionDataReal.emojiId;
				}
				else {
					emoji = deleteReactionDataReal.emojiName;
				}
				CURL* curl = curl_easy_init();
				char* output = nullptr;
				if (curl) {
				...
				string emojiEncoded = output;
				deleteReactionDataReal.encodedEmoji = emojiEncoded;
				DataManipFunctions::deleteObjectDataAsync(this->pHttpController, deleteReactionDataReal).get();
				co_return;
			}

			task<void> deleteOwnReactionAsync(DataManipFunctions::DeleteOwnReactionData deleteReactionData) {
				DataManipFunctions::DeleteOwnReactionData deleteReactionDataReal = deleteReactionData;
				deleteReactionDataReal.channelId = this->channelId;
				deleteReactionDataReal.messageId = this->messageId;
				string emoji;
				if (deleteReactionDataReal.emojiId != string()) {
		...
		public:
			...
			task<Message> fetchAsync(string messageId) {
				try {
					...
					cout << "MessageManager.fetchAsync() Error: " << e.what() << endl;
				}
				
			};
			
			task<Message> GetMessageReal(string messageId) {
