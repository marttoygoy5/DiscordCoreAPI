...
namespace CommanderNS {
	...
	public:
		...
		DiscordCoreAPI(hstring botToken) {
			...
			this->webSocketConnection->initialize(botToken, this->eventMachine, this->systemThreads, this->restAPI, this->client);
			this->webSocketReceiver->initialize(botToken, this->eventMachine, this->systemThreads, this->restAPI, this->client);
			this->pHttpHandler = make_self<HTTPHandler>(this->restAPI);
			this->client = make_self<ClientClasses::Client>(this->pHttpHandler, this->systemThreads);
	...
	protected:
		...
		com_ptr<HTTPHandler> pHttpHandler{ nullptr };
